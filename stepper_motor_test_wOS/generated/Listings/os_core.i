...
int OS_AddThreads(void(*thread0)(void), uint32_t p0,
  ...
  status = StartCritical(); 
	...
	for(i=0; i< 8; i++){tcbs[i].blocked = 0;}
...
void static runsleep(void){
	...
	uint8_t i;
	for (i=0;i<8;i++){ if(tcbs[i].sleep != 0) {	
			tcbs[i].sleep --;	
		}
	}
}
